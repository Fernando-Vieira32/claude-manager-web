...
| `test/conversations-messages.test.js` | leitura do histórico em **blocos**, na ordem: prosa e chamada da mesma mensagem virando dois blocos, `tool_result` costurado pelo id, mensagem só-de-ferramenta não descartada, resultado órfão, várias ferramentas numa mensagem — **e o agente**: bloco próprio (não ferramenta), o aceite do disparo NÃO o encerrando, o `<task-notification>` entregando o relatório no bloco dele dez minutos depois (com a duração tirada dos dois carimbos), agente que falhou marcado, aviso de agente de outra conversa não quebrando a leitura, agente síncrono terminando pelo próprio resultado, **status de fim que não é `completed`** (`killed`/`stopped` encerrando, status desconhecido também encerrando, e só `running` mantendo de pé) — **e a conciliação pelo contador**: contador ausente depois de o arquivo já ter mostrado o campo conta como zero (era o agente com relógio de 6 dias), mas arquivo que nunca mostrou o campo não decide nada (a trava que impede apagar da tela quem está trabalhando) — **e quem escreveu a mensagem**: a saída de um comando local (`/context`) vem com `fromCli: true`, o que a pessoa digitou vem `false`, e a resposta do Claude também (ela é formatada pelo papel, não por marca) |
...
| `test/markdown.test.js` | a tradução **markdown → blocos** que a resposta do Claude atravessa antes de virar tela (`public/js/core/markdown.js` + `markdown-inline.js`): título (e `#3` de issue **não** sendo título), cerca guardando a linguagem e o conteúdo exato — inclusive **cerca aberta**, que é o estado normal durante o stream —, lista aninhada por indentação (item mais raso encerrando a sub-lista, troca de marcador começando outra lista, código dentro do item), citação lida como markdown, régua, tabela pelo separador (e `ps aux \| grep node` **não** virando tabela), parágrafo parando onde outro bloco começa e preservando a quebra escrita; na linha: negrito/itálico/riscado/link/url solta, aninhamento (`**com \`código\` dentro**`), barra invertida, marcação aberta a meio do stream ficando texto, `\`ls **/*.rb\`` intacto porque código tem prioridade, e **`_` dentro de palavra não sendo itálico** (`built_in_copy_spec.rb`) — mais a mensagem real do print, na íntegra, conferida bloco a bloco |
| `test/conversations-subagent.test.js` | os **passos** de um subagente (`services/conversations/subagent.js` + `core/claude-paths.js`): o caminho do arquivo do agente ao lado da conversa, id de agente recusado quando tenta escapar da pasta, os passos vindo como blocos, o `meta` do CLI, o teto devolvendo as **últimas** entradas com o `from` (para a tela dizer que veio cortado), meta ausente não derrubando a leitura, tradução do id do **disparo** pelo `toolUseId`, arquivo já limpo virando erro que diz a verdade — **e o julgamento do agente aberto** (`settleOpenAgents`, o que mata o fantasma de 19 dias): transcrito que termina falando encerra o agente e o texto dele vira o relatório, os dois sinais de "estava no meio" testados **separados** (último bloco é chamada; `stop_reason: tool_use` com fala por último), sem transcrito não decide nada, e conversa sem agente aberto não vai ao disco |
...
| `test/chat-args.test.js` | os modos de permissão (`services/chat/args.js`): cada modo virando a flag certa do CLI (`plan`/`auto`/`acceptEdits`/`bypassPermissions`), os três que editam/executam **recusando** sem `CHAT_ALLOW_FULL_TOOLS=1`, modo desconhecido recusado em vez de cair no padrão do CLI — e a **trava**: fora do "só conversa", nenhum modo passa `--disallowedTools`/`--allowedTools`/`--tools`, para o navegador nunca voltar a ser mais restrito que o terminal por bloqueio nosso |
...
O **markdown** dividiu-se do mesmo jeito, e é o exemplo mais limpo do padrão: a gramática
é pura (`public/js/core/markdown.js` + `markdown-inline.js`) e virou teste de verdade na
suíte; o desenho (`components/markdown-text.js`) foi conferido por arnês, montando a bolha
real com a mensagem do print que motivou a mudança. O que o arnês provou e a suíte não
alcança: negrito/tabela/lista/cerca virando as tags certas, link `javascript:` saindo como
texto riscado sem `href`, bolha do **usuário** continuando `<pre>` (texto cru), dois deltas
seguidos agendando **um** quadro só (o coalescimento), `finish()` não deixando quadro
pendente e bolha vazia ainda dizendo "(sem texto)".
